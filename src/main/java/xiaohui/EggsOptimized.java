...
public class EggsOptimized {
	...
	public int getMinSteps(int eggNum, int floorNum) {
		...
		int[] currentCache = new int[floorNum + 1];/*把备忘录每个元素初始化成最大的尝试次数*/
		for (int i = 1; i <= floorNum; i++)
			currentCache[i] = i;
		for (int n = 2; n <= eggNum; n++) {/*当前备忘录拷贝给上一次备忘录，并重新初始化当前备忘录*/
			preCache = currentCache.clone();
			for (int i = 1; i <= floorNum; i++)
				currentCache[i] = i;
			for (int m = 1; m <= floorNum; m++)
				for (int k = 1;
					 k < m; k++) {/*扔鸡蛋的楼层从1到m枚举一遍，如果当前算出的尝试次数小于上一次算出的尝试次数，则取代上一次的尝试次数。*//*这里可以打印k的值，从而知道第一个鸡蛋是从第几次扔的。*/
